...
class ConnectSendViewController: BaseViewController {
    ...
    @IBOutlet weak var stepLimitInfoButton: UIButton! // 굳이?
    ...
    func generateTransaction() {
        ...
        guard let tx = transaction, let estimatedStepCost = estimateStepLimit(tx: tx) else {
            return
        }
        tx.stepLimit(estimatedStepCost)
    }
    
    func estimateStepLimit(tx: Transaction) -> BigUInt? {
        let estimate = WManager.service.estimateStep(transaction: tx).execute()
        
        switch estimate {
        case .success(let step):
            // Set some margin
            let realEstimatedStep = step + 20000
            self.stepLimitLabel.text = String(realEstimatedStep)
            self.estimatedStep = realEstimatedStep
            return realEstimatedStep
        case .failure(let err):
            Conn.sendError(error: ConnectError.network(err))
            return nil
        }
    }
    ...
    func validateLimit(_ showError: Bool = true) -> Bool {
        ...
        guard let limit = self.estimatedStep else {
            let limit = BigUInt(0)
