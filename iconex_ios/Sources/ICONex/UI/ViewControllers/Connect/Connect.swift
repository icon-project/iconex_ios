...
class Connect {
    ...
    public func translate() throws {
        ...
        if Conn.action == "JSON-RPC" {
            ...
            DispatchQueue.global().async {
                guard let requestSymbol = Manager.icon.getIRCTokenInfo(walletAddress: from, contractAddress: to) else { return }
                Conn.tokenSymbol = requestSymbol.symbol
                Conn.tokenDecimal = Int(requestSymbol.decimal)
            }
        }
    }
    ...
    public func sendError(error: ConnectError) {
        guard Conn.redirect != nil else {
            let app = UIApplication.shared.delegate as! AppDelegate
            app.toMain()
            app.window?.showToast(message: error.errorDescription ?? "Unknown Error")
            return
        }
...
class ConnectManager {
    ...
    var provider: ICONService = Manager.icon.service
}
